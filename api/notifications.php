 ...
 * priority categorization, and AI rule-based heuristics.
 */

require_once '../config/cors.php';
require_once '../config/db.php';
require_once '../config/session.php';
require_once '../config/gemini.php'; // Optional: for AI integration if needed in future advanced rules
...
if ($method === 'POST' && $action === 'create') {
    ...
    // Priority Engine Heuristics (Rule-based fallback for AI)
    $priority = sanitize($data['priority'] ?? '');
    if (empty($priority)) {
        $msgLower = strtolower($message);
        $priority = 'normal';
        if (strpos($msgLower, 'accepted') !== false || strpos($msgLower, 'rejected') !== false || strpos($msgLower, 'shortlisted') !== false || strpos($msgLower, 'message') !== false) {
            $priority = 'high';
        } elseif (strpos($msgLower, 'training') !== false || strpos($msgLower, 'workshop') !== false || strpos($msgLower, 'update') !== false) {
            $priority = 'low';
        }
    }
    ...
    try {
        ...
        jsonResponse(['success' => true, 'id' => (int)$db->lastInsertId(), 'priority' => $priority], 201);
    } catch (Exception $e) {
