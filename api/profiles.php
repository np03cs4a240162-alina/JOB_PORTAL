...
if ($method === 'GET') {
    ...
    if ($targetRole === 'seeker') {
        $stmt = $db->prepare('SELECT phone, skills, experience, bio, photo, parsed_skills, parsed_education, parsed_experience, resume_file_path FROM seeker_profiles WHERE user_id = ?');
...
if ($method === 'POST' || $method === 'PUT') {
    ...
    if ($user['role'] === 'seeker') {
        ...
        logActivity($user['id'], $user['name'], $user['role'], 'Updated Seeker Profile details', 'Updated phone, skills, experience, and bio');
    ...
    if ($user['role'] === 'employer') {
        ...
        logActivity($user['id'], $user['name'], $user['role'], 'Updated Employer Corporate Profile', 'Updated company name, industry sector, corporate website, and about details');
...
if ($method === 'PATCH') {
    ...
    try {
        ...
        logActivity($user['id'], $user['name'], $user['role'], 'Updated core Account details', "Name: " . $name . " | Email: " . $email);
