...
function verifyOtp(string $email, string $otp): bool {
    if (empty($_SESSION['otp'])) return false;
...
function sendOtpEmail(string $toEmail, string $otp): bool {
    ...
    if ($isLocal) {
        // Log the OTP to a text file in your project root so you can read it!
        $logMessage = "[" . date('Y-m-d H:i:s') . "] To: $toEmail | OTP: $otp" . PHP_EOL;
        file_put_contents(__DIR__ . '/../otp_debug.txt', $logMessage, FILE_APPEND);
        
        // Return true immediately so the registration doesn't hang/crash
        return true; 
    }
    ...
    // Use @ to suppress XAMPP mail server warnings
